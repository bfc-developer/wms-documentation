# Sync Karvy Liquidated Schemes

This API identifies and synchronizes liquidated schemes from Karvy portfolios. It isolates liquidated folios, calculates their detailed performance metrics using FIFO logic, and updates the central portfolio collection with the finalized data.

### User flow diagram

```mermaid
graph TD
    Start([Start]) --> MatchLiquidated["Match Liquidated Status in foliok"]
    ...
    LookupNav --> LookupTrans["Lookup Transactions (trans_karvy)"]
