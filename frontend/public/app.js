class DocumentStore {
    ...
    openDocumentTree(path) {
        const treeNodes = document.querySelectorAll('.tree-node');
        let i;
        for (i = 0; i < treeNodes.length; i++) {
            const n = treeNodes[i];
            if (n.dataset.type == 'directory' && path.startsWith(n.dataset.path)) {
                this.toggleDirectory(n);
            }
            if(n.dataset.type == 'file' && path == n.dataset.path) {
                this.setActiveNode(n);
                return; 
            }
        }
    }
